<!DOCTYPE html>
<html>
<head>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.6.1/dist/css/bootstrap.min.css" integrity="sha384-zCbKRCUGaJDkqS1kPbPd7TveP5iyJE0EjAuZQTgFLD2ylzuqKfdKlfG/eSrtxUkn" crossorigin="anonymous">
</head>
<title>ADMIN CHATBOT SERVICE LAPTOP</title>
<body>
    <nav class="navbar navbar-dark bg-dark">
            <span class="navbar-brand mb-0 h1">ADMIN CHATBOT SERVICE LAPTOP</span><br><a href="logout.php">Logout</a>
        </div>
    </nav>
<div class="container">
    <br>
    <h4><center>UPDATE RESPONSES</center></h4>
<?php

    include "koneksii.php";

    //Cek apakah ada kiriman form dari method post
    if (isset($_GET['id'])) {
        $id=htmlspecialchars($_GET["id"]);

        $sql="delete from chatbot where id='$id' ";
        $cek_data=mysqli_query($kon,$sql);

        //Kondisi apakah berhasil atau tidak
            if ($cek_data) {
                header("Location:indexx.php");

            }
            else {
                echo "<div class='alert alert-danger'> Data Gagal dihapus.</div>";

            }
        }
?>


     <tr class="table-danger">
            <br>
        <thead>
        <tr>
       <table class="my-3 table table-bordered">
            <tr class="table-primary">           
            <th>No</th>
            <th>Pertanyaan</th>
            <th>Jawaban</th>
            <th colspan='2'>Aksi</th>

        </tr>
        </thead>

        <?php
        include "koneksii.php";
        $sql="select * from chatbot order by id desc";

        $cek_data=mysqli_query($kon,$sql);
        $no=0;
        while ($data = mysqli_fetch_array($cek_data)) {
            $no++;

            ?>
            <tbody>
            <tr>
                <td><?php echo $no;?></td>
                <td><?php echo $data["pertanyaan"]; ?></td>
                <td><?php echo $data["jawaban"];   ?></td>
                <td>
                    <a href="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>?id=<?php echo $data['id']; ?>" class="btn btn-danger" role="button">Delete</a>
                </td>
            </tr>
            </tbody>
            <?php
        }
        ?>
    </table>
    <a href="create.php" class="btn btn-primary" role="button">Tambah Data</a>
</div>
</body>
</html>
